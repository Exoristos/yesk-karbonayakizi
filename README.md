...
## 📂 Dosya Yapısı
* `app.py`: Ana Streamlit uygulaması (sekme düzeni, story kartı; grafikler `yesk/charts.py`).
* `yesk/calc.py`: Günlük kg CO₂ hesabı ve treemap satırları.
* `yesk/data_io.py`: `data/complex_*.json|csv` yükleme ve şema doğrulama.
* `yesk/stats.py`, `yesk/charts.py`, `yesk/ui/styles.py`: kohort yüzdelik, Plotly şekilleri, CSS.
* `requirements.txt`: Streamlit Cloud'un projeyi ayağa kaldırması için gereken kütüphane listesi.
* `data/README.md`: Canlı veri dosyaları ve kullanılmayan eski CSV’ler hakkında notlar.
* `data/`
...
* `scripts/archive/update_app_legacy.py`: **Kullanmayın** — tarihî, kırılgan `app.py` yamalayıcısı.
